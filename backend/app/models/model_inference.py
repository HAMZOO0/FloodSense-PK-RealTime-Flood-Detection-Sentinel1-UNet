...
def resolve_weights_path(weights_path=None):
    ...
    raise FileNotFoundError("Model weights not found. Place best_flood_model.pth in models/")
...
def normalize_image(img):
    ...
    out[0] = (np.clip(img[4], -40, 10) + 40) / 50.0
    out[1] = (np.clip(img[5], -45, -5) + 45) / 40.0
    vv = out[0] + 1e-8
    vh = out[1] + 1e-8
    out[2] = np.clip(vh / vv, 0, 2) / 2.0
...
def preprocess_image_tile(vv_tile):
    ...
    img_norm = np.stack([ch0, ch1, ratio], axis=-1).astype(np.float32)
    transform = ToTensorV2()
    tensor = transform(image=img_norm)["image"].unsqueeze(0)
    return tensor
        ...
        for x in range(0, w_full, tile_size):
            ...
            if vv_tile.shape[0] != tile_size or vv_tile.shape[1] != tile_size:
                vv_tile = cv2.copyMakeBorder(
                    vv_tile,
                    0,
                    tile_size - vv_tile.shape[0],
                    0,
                    tile_size - vv_tile.shape[1],
                    cv2.BORDER_CONSTANT,
                    value=0,
                )
