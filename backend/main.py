...
from app.engine.data_manager import save_json
from app.models.model_inference import load_flood_model, predict_flood, resolve_weights_path
from app.scrapers.ffc_scraper import get_ffc_data
from app.utils.districts import PRIORITY_DISTRICTS, detect_name_column, flood_percent_for_district, shapely_to_ee
from app.utils.ndwi import get_flood_mask
from app.utils.visualize import plot_static_map
...
def fetch_current_sar_image(bbox, date_start, date_end, size=256):
    ...
    url = masked_image.getThumbURL({"region": bbox, "dimensions": size, "format": "png", "min": -25, "max": 0})
...
def parse_args():
    ...
    parser.add_argument("--quick", action="store_true", help="Process only 4 demo districts (faster for testing)")
    parser.add_argument("--skip-historical", action="store_true", help="Skip 2010 Landsat baseline (GEE heavy); current flood % only")
    parser.add_argument("--current-start", type=str, default=None, help="Current SAR date range start (YYYY-MM-DD). Default: last 30 days.")
    parser.add_argument("--current-end", type=str, default=None, help="Current SAR date range end (YYYY-MM-DD). Default: today.")
...
def main():
    ...
    date_end = datetime.strptime(args.current_end, "%Y-%m-%d") if args.current_end else datetime.now()
    date_start = datetime.strptime(args.current_start, "%Y-%m-%d") if args.current_start else date_end - timedelta(days=30)
    ...
    mask = gdf[name_col].apply(lambda x: any(p.lower() in str(x).lower() for p in priority))
    gdf_priority = gdf[mask].copy()
    log(f"        Targeting {len(gdf_priority)} districts" + (" (quick mode)" if args.quick else "") + ".")
    ...
    for n, (_, row) in enumerate(gdf_priority.iterrows(), start=1):
        ...
        try:
            ...
            pct_2010 = flood_percent_for_district(ee_geom, hist_flood_mask) if hist_flood_mask is not None else None
            log("       Live SAR + U-Net...")
